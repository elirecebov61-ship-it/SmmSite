...
def wico(size=16, color="#3B82F6", mr=0):
    """Bütün saytda tutarlı istifadə olunan mavi 'pəncərə' ikonu (SVG).
    Emoji fontundan asılı olmadığı üçün heç bir cihazda/brauzerdə boş
    kvadrat (tofu) kimi görünmür — həmişə eyni cür render olunur."""
    mrg = f"margin-right:{mr}px;" if mr else ""
    return (f'<svg width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" '
            f'xmlns="http://www.w3.org/2000/svg" style="flex-shrink:0;display:inline-block;'
            f'vertical-align:-3px;{mrg}"><rect x="3" y="4" width="18" height="16" rx="3" '
            f'stroke="{color}" stroke-width="2"/><line x1="3" y1="9.5" x2="21" y2="9.5" '
            f'stroke="{color}" stroke-width="2"/><circle cx="6.5" cy="6.7" r="0.9" fill="{color}"/>'
            f'<circle cx="9.3" cy="6.7" r="0.9" fill="{color}"/></svg>')
...
<script>
(function(){{
  var eb=document.getElementById('order-err-box');
  if(eb){{ setTimeout(function(){{ eb.scrollIntoView({{behavior:'smooth',block:'center'}}); }},60); }}
}})();
var allProds={{}};
