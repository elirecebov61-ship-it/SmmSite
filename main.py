...
.sb-head .sb-bal{margin-top:10px;font-size:14px;font-weight:800;color:#fff;background:linear-gradient(90deg,var(--or),var(--ord));display:flex;align-items:center;gap:6px;width:fit-content;padding:7px 16px;border-radius:999px;position:relative;overflow:hidden;box-shadow:0 4px 14px rgba(0,0,0,.25),inset 0 1px 0 rgba(255,255,255,.25);border:1px solid rgba(255,255,255,.3);}
...
<div class="sidebar" id="sb">
  <div class="sb-head">
    ...
    <div class="sb-bal">{wico(15,"#fff",6)}Balans: {bal:.2f} ₼</div>
...
<td><a href="{buy_url}" class="btn-al">{wico(13,"#fff",5)}Al!</a></td>
    ...
    if not rows:
        rows = f"""<tr><td colspan="7" style="padding:0;border:none;">{empty_state("🔍","Xidmət tapılmadı","Axtarışınıza uyğun xidmət tapılmadı, başqa açar söz yoxlayın.")}</td></tr>"""

    body = f"""<section class="wrap" style="padding:24px 0 60px;">
  <div class="table-card">
    <div class="search-bar" style="margin:16px;">
      <span class="search-bar-ic">{wico(18)}</span>
      <form method="get" action="/services" style="display:flex;width:100%;">
        <input type="text" name="q" value="{q}" placeholder="Xidmət axtar...">
      </form>
    </div>
...
def services_page(request: Request, q: str = ""):
    ...
    try:
        c=conn.cursor(cursor_factory=RealDictCursor)
        c.execute("SELECT * FROM products WHERE stock>0 ORDER BY product_id")
        prods=c.fetchall()
    finally: put_conn(conn)
    bal_info=get_balance(user["email"])
