...
/* SIDEBAR */
.sb-ov{display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.6);backdrop-filter:blur(2px);z-index:52;}
.sb-ov.open{display:block;animation:pageFadeIn .2s ease;}
.sidebar{position:fixed;top:0;left:-300px;width:280px;height:100%;background:rgba(255,255,255,.98);backdrop-filter:blur(20px) saturate(140%);-webkit-backdrop-filter:blur(20px) saturate(140%);border-right:1px solid rgba(0,0,0,.08);z-index:53;transition:left .32s cubic-bezier(.4,0,.2,1);box-shadow:8px 0 32px rgba(0,0,0,.25);display:flex;flex-direction:column;overflow-y:auto;}
.sidebar.open{left:0;}
...
.stat-info .stat-label{font-size:12px;color:var(--mu);margin-top:3px;display:flex;align-items:center;}
...
.plat-tab{display:flex;align-items:center;justify-content:center;gap:9px;background:var(--sf);border:1.5px solid var(--ln);border-radius:12px;padding:15px 16px;font-size:15px;font-weight:700;cursor:pointer;color:var(--tx);transition:all .15s;font-family:'Inter',sans-serif;}
...
function pjaxGo(url, push){
  pjaxFetch(url, {method:'GET'}, push!==false);
}

function pjaxFetch(url, opts, push){
  ...
  fetch(url, Object.assign({credentials:'same-origin'}, opts||{}))
    ...
    .catch(function(){ __pjaxBusy=false; window.location.href=url; });
}
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  document.body.addEventListener('submit',function(e){
    ...
    if(method==='get'){
      ...
      pjaxFetch(url.href, {method:'GET'}, true);
    }else{
      var fd2=new FormData(f);
      pjaxFetch(url.href, {method:'POST', body:fd2}, true);
    }
  });

  window.addEventListener('popstate',function(){
    pjaxFetch(location.href, {method:'GET'}, false);
  });
});
...
def wico(size=16, color="#3B82F6", mr=0):
    """Bütün saytda tutarlı istifadə olunan sadə düzbucaqlı 'pəncərə' ikonu.
    Bu, real şəkil (SVG) olduğu üçün heç bir cihazda/brauzerdə (emoji şrifti
    olsun-olmasın) boş kvadrat kimi sınmır — həmişə eyni cür görünür."""
...
def sidebar_html(user, bal_info, active=""):
    ...
    for ic,lbl,href,key,bd in links:
        ...
        nav += f'<a href="{href}" class="{cls}"><span class="icon">{wico(18)}</span>{lbl}{bd}</a>'
...
<div class="stat-card">
  <div class="stat-icon-wrap"><div class="stat-icon">{wico(24)}</div></div>
  <div class="stat-info">
    <div class="stat-name" style="color:var(--or);">{name}</div>
    <div class="stat-label">{wico(13,mr=5)}Xoş Gəlmisiniz</div>
  </div>
</div>
<div class="stat-card">
  <div class="stat-icon-wrap"><div class="stat-icon">{wico(24)}</div></div>
  <div class="stat-info">
    ...
    <div class="stat-label">{wico(13,mr=5)}Ümumi Xərclənən</div>
  </div>
</div>
<div class="stat-card">
  <div class="stat-icon-wrap"><div class="stat-icon">{wico(24)}</div></div>
  <div class="stat-info">
    ...
    <div class="stat-label">{wico(13,mr=5)}Ümumi Sifarişlər</div>
  </div>
</div>
<div class="stat-card">
  <div class="stat-icon-wrap"><div class="stat-icon">{wico(24)}</div></div>
  <div class="stat-info">
    ...
    <div class="stat-label">{wico(13,mr=5)}Balans</div>
  </div>
</div>
</div>"""
...
def render_orders(user, bal_info, orders, new=False):
    ...
    if not orders:
        ...
        for o in orders:
            ...
            if status=="Gözləmədə":
                action_html=f'<button type="button" class="btn-row-action btn-row-cancel" onclick="cancelOrder(this,{oid})">Ləğv et</button>'
            elif status=="Tamamlandı":
                action_html=f'<button type="button" class="btn-row-action btn-row-restore" onclick="restoreOrder(this,{oid})">Bərpa et</button>'
            else:
                action_html="—"
