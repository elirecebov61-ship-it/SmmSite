...
[data-theme="dark"] .topbar{background:linear-gradient(135deg,rgba(30,58,138,.55),rgba(11,10,18,.85));}
...
/* TOPBAR */
.topbar{background:linear-gradient(135deg,rgba(219,234,254,.92),rgba(191,219,254,.85));backdrop-filter:blur(16px) saturate(140%);-webkit-backdrop-filter:blur(16px) saturate(140%);border-bottom:1px solid var(--glass-brd);position:sticky;top:0;z-index:50;box-shadow:0 4px 24px rgba(59,130,246,.15);}
...
.brand-text-logo{font-weight:900;font-size:26px;letter-spacing:-.03em;color:#2563EB;}
...
LOGO_SVG = '<span class="notranslate brand-text-logo">PanelimAz</span>'
...
def create_order(request: Request, product_id:int=Form(...), quantity:int=Form(...), profile_link:str=Form(...)):
    ...
    try:
        ...
        if bal<total:
            ...
            return RedirectResponse(url=f"/?order_err={quote('Balansda kifayət qədər vəsait yoxdur')}",status_code=status.HTTP_303_SEE_OTHER)
