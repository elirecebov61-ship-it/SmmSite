...
function initOrderPriceCalc(){
  ...
  function updatePrice(){
    ...
    if(qtyRaw===''){ pi.innerText=''; if(etaEl)etaEl.value=''; checkAll(); return; }
...
<div class="order-panel">
  ...
  <form method="post" action="/order" id="order-form">
    ...
    <div class="field"><label>Miqdar</label>
      <input type="number" id="qty-input" name="quantity" placeholder="Məs: 1000" min="1" required>
...
def render_orders(user, bal_info, orders, new=False):
    ...
    if not orders:
        ...
        for o in orders:
            ...
            can_cancel = status in ("Gözləmədə","Davam Edir")
            can_restore = status in ("Tamamlandı","Ləğv Edildi")
            btns=[]
            if can_cancel:
